...
public class EditUserPageTest extends AbstractPageTest {
    ...
    private void startPageForUser(String otpTypeCode, boolean isOtpOptional) {
        ...
        // the form requires these, so fill them in to keep a submit from failing validation
        user.setEmail("john@example.com");
        user.setName("John");
        user.setSurname("Doe");
        user.setSecretQuestion("q");
        user.setSecretAnswer("a");
    ...
    /**
     * The ajax behavior fires on a change only, so a user already stored with mandatory OTP
     * and no type would otherwise be saved back unchanged.
     */
    @Test
    public void savingWithoutTouchingTheControlsStillSetsGoogleAuth() {
        Capture<UIUser> saved = EasyMock.newCapture();
        expect(userService.updateUser(EasyMock.capture(saved))).andReturn(RoutineResult.okResult());
        startPageForUser(OTPType.NONE.code(), false);

        // onSubmit redirects to ListUsersPage, whose own beans are out of scope here
        tester.setExposeExceptions(false);
        tester.newFormTester("form").submit();

        assertEquals(OTPType.GOOGLE_AUTH.code(), saved.getValue().getOtpType());
    }
}
