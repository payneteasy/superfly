...
delimiter $$
create procedure login_locked(i_user_name varchar(32), i_max_logins_failed int(10), i_lockout_type varchar(255))
main_sql:
  begin
    ...
    set v_is_account_locked = 'N';
     ...
     select user_id, is_account_locked,
     	case
     		when i_lockout_type = 'PASSWORD' then logins_failed
     		when i_lockout_type = 'HOTP' then hotp_logins_failed
     		else 0 end
